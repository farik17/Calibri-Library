...
auto cipher(const ByteArray &data, const ByteArray &key, const ByteArray &iv, bool *ok = nullptr) noexcept -> ByteArray
{
    try {
        ...
        if (UNLIKELY(cipherAlgorithm == EVP_enc_null())) {
            if (ok)
                *ok = false;
        ...
        if (UNLIKELY(EVP_EncryptInit(&cipherContext, cipherAlgorithm, reinterpret_cast<const uchar *>(key.data()), reinterpret_cast<const uchar *>(iv.data())) != 1)) {
        ...
        if (UNLIKELY(EVP_EncryptUpdate(&cipherContext, reinterpret_cast<uchar *>(encryptedData.data()), &processedBytes, reinterpret_cast<const uchar *>(data.data()), data.size()) != 1)) {
            EVP_CIPHER_CTX_cleanup(&cipherContext);
        ...
        if (UNLIKELY(EVP_EncryptFinal(&cipherContext, reinterpret_cast<uchar *>(encryptedData.data() + encryptedBytes), &processedBytes) != 1)) {
            EVP_CIPHER_CTX_cleanup(&cipherContext);
...
auto cipher(const ByteArray &data, const ByteArray &key, const ByteArray &iv, bool *ok = nullptr) noexcept -> ByteArray
{
    try {
        ...
        if (UNLIKELY(EVP_DecryptInit(&cipherContext, cipherAlgorithm, reinterpret_cast<const uchar *>(key.data()), reinterpret_cast<const uchar *>(iv.data())) != 1)) {
            EVP_CIPHER_CTX_cleanup(&cipherContext);
        ...
        if (UNLIKELY(EVP_DecryptUpdate(&cipherContext, reinterpret_cast<uchar *>(decryptedData.data()), &processedBytes, reinterpret_cast<const uchar *>(data.data()), data.size()) != 1)) {
            EVP_CIPHER_CTX_cleanup(&cipherContext);
        ...
        if (UNLIKELY(EVP_DecryptFinal(&cipherContext, reinterpret_cast<uchar *>(decryptedData.data() + decryptedBytes), &processedBytes) != 1)) {
            EVP_CIPHER_CTX_cleanup(&cipherContext);
